...
exports.buildIcsEvent = buildIcsEvent;
exports.buildGoogleCalendarEvent = buildGoogleCalendarEvent;
exports.buildAppleCalendarEvent = buildAppleCalendarEvent;
/**
...
function buildIcsEvent(input) {
    ...
    const ics = [
        ...
        `DESCRIPTION:${escapeIcs(input.description || "Vehicle maintenance event")}`,
        "END:VEVENT",
        "END:VCALENDAR",
    ].join("\r\n");
 ...
 */
function buildGoogleCalendarEvent(input) {
    const eventId = `${Date.now()}-${input.vehicleVin}-google`;
    const startTs = toIcsTimestamp(input.startAt);
    const endTs = toIcsTimestamp(input.endAt || input.startAt);
    const actionUrl = "https://calendar.google.com/calendar/render?action=TEMPLATE" +
        `&text=${encodeURIComponent(input.title)}` +
        `&dates=${encodeURIComponent(`${startTs}/${endTs}`)}` +
        `&details=${encodeURIComponent(input.description || "Vehicle maintenance event")}`;
    return {
        eventId,
        actionUrl,
    };
 ...
 */
function buildAppleCalendarEvent(input) {
