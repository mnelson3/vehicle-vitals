...
class DataExportService {
  ...
  Future<void> exportMaintenanceAsCSV(String vin) async {
    ...
    await SharePlus.instance.share(
      ShareParams(
        ...
        subject: 'Vehicle Vitals Maintenance Export (Vehicle ID: $vin)',
        text: 'Maintenance export generated by Vehicle Vitals.',
      ),
    );
  }

  Future<void> exportMaintenanceAsExcel(String vin) async {
    final maintenanceData = await _getMaintenanceData(vin);
    final vehicle = await _getVehicle(vin);
    final workbook = Excel.createExcel();
    final sheet = workbook['maintenance'];
    ...
    for (final entry in maintenanceData) {
      sheet.appendRow([
        TextCellValue(vin),
        TextCellValue((vehicle?['make'] ?? '').toString()),
        TextCellValue((vehicle?['model'] ?? '').toString()),
        ...
        TextCellValue(_isoDate(entry['updatedAt'])),
      ]);
    }

    final exportDir = await _exportDirectory();
    final fileName =
        'maintenance-$vin-${DateTime.now().millisecondsSinceEpoch}.xlsx';
    final file = File('${exportDir.path}/$fileName');
    ...
    await SharePlus.instance.share(
      ShareParams(
        files: [XFile(file.path)],
        subject: 'Vehicle Vitals Maintenance Export (Vehicle ID: $vin)',
        text: 'Maintenance Excel export generated by Vehicle Vitals.',
      ),
    );
  }
  ...
  Future<void> exportMaintenanceAsPDF(String vin) async {
    ...
    await SharePlus.instance.share(
      ShareParams(
        files: [XFile(file.path)],
        subject: 'Vehicle Vitals Maintenance Report (Vehicle ID: $vin)',
    ...
    for (final vehicleDoc in vehiclesSnapshot.docs) {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (context) {
            return [
              pw.Header(level: 0, child: pw.Text('Vehicle ID - $vin')),
              pw.Text(
                'Vehicle: ${vehicle['year'] ?? ''} ${vehicle['make'] ?? ''} ${vehicle['model'] ?? ''}',
              ),
