...
function MaintenanceList({ vin }: { vin: string }) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadPromises = files.map(async (file) => {
        ...
        const path = await generateMaintenanceAttachmentPath(vin, tempId, file.name);
  ...
  const upcomingMaintenance = vehicle ? getUpcomingMaintenance(vehicle.make, vehicle.model, vehicle.mileage) : [];
    ...
    <div className="bg-white dark:bg-charcoal-800 rounded-lg shadow-md p-6">
      ...
      {vehicle && (
        <div className="mb-6 border-b border-charcoal-200 dark:border-charcoal-600 pb-4">
          <h4 className="font-serif font-bold text-xl text-charcoal-800 dark:text-cream-100 mb-3">Recommended Maintenance</h4>
          <p className="text-sm text-charcoal-600 dark:text-cream-300 mb-3">
            {vehicle.make} {vehicle.model} ({vehicle.year}) • Current mileage: {vehicle.mileage}
          </p>
          {upcomingMaintenance.length > 0 ? (
            <div className="space-y-2">
              {upcomingMaintenance.slice(0, 3).map((item, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-charcoal-50 dark:bg-charcoal-700 rounded-md">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-oxblood-100 dark:bg-oxblood-900 rounded-full flex items-center justify-center">
                      <span className="text-xs font-bold text-oxblood-600 dark:text-oxblood-300">!</span>
                    </div>
                    <div>
                      <div className="font-medium text-charcoal-800 dark:text-cream-100">{item.description}</div>
                      <div className="text-sm text-charcoal-600 dark:text-cream-300">
                        Due: {item.nextDueMileage} miles ({item.milesUntilDue} miles)
                      </div>
                    </div>
                  </div>
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-serif font-bold text-xl text-charcoal-800 dark:text-cream-100">Maintenance History</h3>
      ...
      <div className="border-t border-charcoal-200 dark:border-charcoal-600 pt-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            />
            {uploading && (
              <p className="text-sm text-charcoal-600 dark:text-cream-300 mt-1">Uploading files...</p>
            )}
          </div>
          {form.attachments && form.attachments.length > 0 && (
            <div>
              <h5 className="text-sm font-medium text-charcoal-700 dark:text-cream-200 mb-2">Attachments</h5>
              <div className="space-y-2">
                {form.attachments.map((attachment, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-charcoal-50 dark:bg-charcoal-700 rounded-md">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-charcoal-800 dark:text-cream-100">{attachment.name}</span>
                    </div>
                    <button
                      type="button"
