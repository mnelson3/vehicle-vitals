...
# Visit: https://github.com/mnelson3/vehicle-vitals/settings/actions/runners
# Click "New self-hosted runner" and copy the token

# Configure the runner (in isolated directory)
cd ~/actions-runner-vehicle-vitals/  # Outside project directory
./config.sh \
  --url https://github.com/mnelson3/vehicle-vitals \
  --token AIQEPB7NN7HGN5V4KR7OZLTJBZYMW \
  --labels "self-hosted,macos-latest,macos-arm64,vehicle-vitals" \
  --name "vehicle-vitals-macos-runner-$(hostname)"
```
...
export RUNNER_DIR="~/actions-runner-vehicle-vitals"  # Outside project directory
export REPO_URL="https://github.com/mnelson3/vehicle-vitals"  # Repository URL
```
...
- `vehicle-vitals` - Project specific
...
# View recent logs
tail -f ~/actions-runner-vehicle-vitals/runner.log
...
1. **Runner won't start**
   ...
   ```bash
   # Check logs
   cat ~/actions-runner-vehicle-vitals/runner.log
...
4. **Permission issues**
   ...
   ```bash
   # Fix permissions
   sudo chown -R $USER:admin ~/actions-runner-vehicle-vitals
   ```
...
ls -la ~/actions-runner-vehicle-vitals/_diag/
